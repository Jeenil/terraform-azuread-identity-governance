...
- **Direct (one-off) assignments** - list specific users in `direct_assignments` to grant a package directly (AdminAdd) for cases the auto-assignment OData filter cannot express; idempotent create and revoke-on-removal
...
### Direct (one-off) user assignments

`auto_assignment_policy` grants a package to everyone matching an OData filter the dept-wide
population. Some grants can't be expressed as a filter: a contractor in another department, a
temporary backfill, a single exception. List those users in a package's `direct_assignments`:
...
access_packages = [
  {
    ...
    # The department is granted by the filter…
    auto_assignment_policy = { dept_name = "Field Operations" }

    # …and these specific users are granted directly (one-offs the filter misses).
    direct_assignments = [
...
**Why this exists:** the `azuread` provider has no resource for an admin-initiated package
assignment, and an auto-assignment policy can only target a filter, not named individuals. The
module fills the gap by resolving each user principal name to an object ID
(`data.azuread_user`) and submitting a Graph **AdminAdd** assignment request through the
package's request-based policy admin-initiated, so it bypasses approval and requestor-scope
restrictions.

**How it behaves:**

- **Idempotent.** Implemented with the same `null_resource` + `local-exec` check-before-create
  pattern as catalog onboarding. The provisioner runs only when an entry is added/removed or the
  resolved user/package/policy id changes; before assigning it queries Graph for an existing
  active assignment and skips if present. A steady-state `terraform apply` issues **no** Graph
  calls for already-assigned users and never creates a duplicate request.
- **Revoke on removal.** Dropping a user from `direct_assignments` (or destroying the package)
  runs a destroy-time AdminRemove for that user's active assignment, so the list is true desired
  state for these grants.
- Same `bash`, `curl`, `jq`, and `ARM_*` requirements as the other `local-exec` blocks; the
  AdminRemove path also falls back to the `az` CLI for a token.

**Required permission:** the AdminAdd/AdminRemove requests need only
`EntitlementManagement.ReadWrite.All` (plus the caller being in a supported Entitlement
Management admin role). Resolving the UPNs to object IDs via `data.azuread_user`, however, is a
**directory read** the caller also needs `User.Read.All` (`User.ReadBasic.All` is not enough;
the data source reads extended attributes). Without it the plan fails on the user lookup with
`403 Authorization_RequestDenied`, even though the assignment itself would be allowed. Omit
`direct_assignments` entirely and no `azuread_user` read (and no `User.Read.All` requirement)
occurs.

This is complementary to and independent of auto-assignment: the policy covers the
department, `direct_assignments` covers the exceptions.
...
| **Direct (one-off) user assignments** | No | Yes - `direct_assignments` AdminAdds named users (idempotent) and revokes them on removal, for cases the auto-assignment filter cannot express |
...
| [null_resource.direct-assignments](https://registry.terraform.io/providers/hashicorp/null/latest/docs/resources/resource) | resource |
